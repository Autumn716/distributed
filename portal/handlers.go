...
func (studentsHandler) renderGrades(w http.ResponseWriter, r *http.Request, id int) {
	...
	res, err := http.Post(fmt.Sprintf("%v/students/%v/grandes", serviceURL, id), "application/json", bytes.NewBuffer(data))
	if err != nil {
		log.Println("Failed to save grade to Grading Service", err)
		return
	}
	if res.StatusCode != http.StatusCreated {
		log.Println("Failed to save grade to Grading Service code: ", res.StatusCode)
		return
	}
}
